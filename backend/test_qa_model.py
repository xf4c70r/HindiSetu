...
def test_qa_model():
    ...
    try:
        ...
        assert 'translation' in processed_data, "Missing translation in response"
        assert 'vocabulary' in processed_data, "Missing vocabulary in response"
        assert isinstance(processed_data['vocabulary'], list), "Vocabulary should be a list"
        if processed_data['vocabulary']:
            assert all(
                isinstance(word, dict) and 
                'word' in word and 
                'meaning' in word and 
                'example' in word
                for word in processed_data['vocabulary']
            ), "Invalid vocabulary structure"
